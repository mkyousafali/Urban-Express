...
Complete PostgreSQL schema for Urban Express with Row Level Security (RLS) policies.

## Schema Overview

The database is designed with security, auditability, and scalability in mind. All sensitive operations are logged, and access is controlled through Row Level Security policies.
...
CREATE OR REPLACE FUNCTION validate_drawer_balance()
RETURNS TRIGGER AS $$
DECLARE
...
BEGIN
  IF NEW.status = 'closing' OR NEW.status = 'closed' THEN
    -- Calculate opening total
    SELECT COALESCE(SUM(cd.value * dod.count), 0) INTO opening_total
    FROM drawer_opening_denominations dod
    JOIN cash_denominations cd ON dod.denomination_id = cd.id
    WHERE dod.drawer_id = NEW.id;
    
    -- Calculate closing total
    SELECT COALESCE(SUM(cd.value * dcd.count), 0) INTO closing_total
    FROM drawer_closing_denominations dcd
    JOIN cash_denominations cd ON dcd.denomination_id = cd.id
    WHERE dcd.drawer_id = NEW.id;
    
    -- Calculate received payments
    SELECT COALESCE(SUM(pcb.denomination_value * pcb.count), 0) INTO received_total
    FROM payment_receipts pr
    JOIN payment_cash_breakdown pcb ON pr.id = pcb.receipt_id
    WHERE pr.drawer_id = NEW.id AND pr.method = 'cash' AND pcb.type = 'received';
    
    -- Calculate change given
    SELECT COALESCE(SUM(pcb.denomination_value * pcb.count), 0) INTO change_total
    FROM payment_receipts pr
    JOIN payment_cash_breakdown pcb ON pr.id = pcb.receipt_id
    WHERE pr.drawer_id = NEW.id AND pr.method = 'cash' AND pcb.type = 'change_given';
    
    -- Expected closing balance
    expected_total := opening_total + received_total - change_total;
    
    -- Calculate variance
    NEW.variance_amount := closing_total - expected_total;
    
    -- Flag significant variances
    IF ABS(NEW.variance_amount) > 1.00 THEN
      NEW.status := 'variance';
    END IF;
  END IF;
...
CREATE TRIGGER cash_drawer_balance_trigger
  BEFORE UPDATE ON cash_drawers
  FOR EACH ROW EXECUTE FUNCTION validate_drawer_balance();
...
INSERT INTO notification_templates (key, name, ar_title, ar_body, en_title, en_body, priority_default) VALUES
('order_placed', 'Order Placed', 'تم تأكيد طلبك', 'تم تأكيد طلبك رقم {{order_number}} بنجاح', 'Order Confirmed', 'Your order {{order_number}} has been confirmed', 'normal'),
('order_out_for_delivery', 'Out for Delivery', 'طلبك في الطريق', 'طلبك رقم {{order_number}} في الطريق إليك', 'Order Out for Delivery', 'Your order {{order_number}} is out for delivery', 'high'),
('delivery_charge_added', 'Delivery Charge Added', 'رسوم توصيل', 'تم إضافة رسوم توصيل {{charge}} ر.س. الإجمالي {{total}} ر.س', 'Delivery Charge Added', 'Delivery charge SAR {{charge}} added. Total SAR {{total}}', 'high'),
('password_reset', 'Password Reset', 'إعادة تعيين كلمة المرور', 'تم إعادة تعيين كلمة المرور الخاصة بك', 'Password Reset', 'Your password has been reset', 'critical');

-- Create default AI policy
INSERT INTO ai_policies (key, name, rules, system_prompt, escalation_keywords) VALUES
...
ARRAY['password', 'refund', 'complaint', 'manager']);
...
This database schema provides a complete foundation for the Urban Express system with proper security, auditability, and performance optimization.